...
namespace Bigint {
	...
	Bigint::Bigint(char* value_s) {
		...
		for (; i <= strlen(value_s) - 1; i++){
			int num;
			//cout << "pos = " << i % 9 << endl;
			//cout << value_s[(strlen(value_s) - 1) - i] << endl;
			num = (value_s[(strlen(value_s) - 1) - i] - '0')*pow(10, (i % 9));
			//cout << num << endl;
			value += num;
			if (i % 9 == 8){
	...
	Bigint Bigint::operator=(const char* s) {
		...
		for (; i <= strlen(s) - 1; i++){
			...
			num = (s[(strlen(s) - 1) - i] - '0')*pow(10, (i % 9));
			//cout << num << endl;
			value += num;
			if (i % 9 == 8){
				//cout << "value =" << value<< endl<<endl;
